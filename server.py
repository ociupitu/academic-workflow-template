...
async def make_api_request(url: str, headers: dict = None, params: dict = None) -> dict[str, Any] | None:
    ...
    # Add Semantic Scholar API key if calling their API
    api_key = os.environ.get("SEMANTIC_SCHOLAR_API_KEY")
    if api_key and SEMANTIC_SCHOLAR_API in url:
...
async def search_papers(query: str, limit: int = 10) -> str:
    ...
    try:
        ...
        semantic_url = f"{SEMANTIC_SCHOLAR_API}/paper/search?query={query}&limit={limit}&fields=title,authors,year,paperId,externalIds,abstract,venue,isOpenAccess,openAccessPdf,tldr"
...
async def fetch_paper_details(paper_id: str, source: str = "semantic_scholar") -> str:
    ...
    """
    if source == "semantic_scholar":
        url = f"{SEMANTIC_SCHOLAR_API}/paper/{paper_id}?fields=title,authors,year,externalIds,abstract,venue,isOpenAccess,openAccessPdf,tldr"
    elif source == "crossref":
